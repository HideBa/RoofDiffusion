    ...
    **modify_kwargs,
    ...
    try:
        ...
        raise NotImplementedError(
            "{} [{:s}() from {:s}] not recognized. Original error: {}".format(
                init_type, class_name, file_name, str(e)
            )
        )
    return ret
...
def parse(args):
    ...
    if args.data_root is not None:
        opt["datasets"][opt["phase"]]["which_dataset"]["args"]["data_root"] = (
            args.data_root
        )
    if args.footprint_root is not None:
        opt["datasets"][opt["phase"]]["which_dataset"]["args"]["footprint_root"] = (
            args.footprint_root
        )
    if args.mask_root is not None:
        opt["datasets"][opt["phase"]]["which_dataset"]["args"]["mask_root"] = (
            args.mask_root
        )
    if args.n_timestep is not None:
        opt["model"]["which_networks"][0]["args"]["beta_schedule"][opt["phase"]][
            "n_timestep"
        ] = args.n_timestep
    if args.sample_num is not None:
        opt["model"]["which_model"]["args"]["sample_num"] = args.sample_num
    if args.use_footprint is not None:
        opt["datasets"][opt["phase"]]["which_dataset"]["args"]["use_footprint"] = (
            args.use_footprint
        )
